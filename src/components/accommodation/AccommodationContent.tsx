...
const AccommodationContent = ({ accommodations }: AccommodationDataProps) => {
  ...
  const [accommodationScrollLists, setAccommodationScrollLists] = useState<
    AccommodationProps[]
  >([]);
  ...
  return (
    <style.AccommodationContentBox>
      <style.AccommodationContentTitleParagraph>
        <span style={{ fontWeight: 600 }}>{accommodations.length}</span>개의
        숙소
      </style.AccommodationContentTitleParagraph>
      <InfiniteScroll
      ...
      >
        <style.AccommodationContentGrid>
          {accommodationScrollLists.length ? (
            accommodationScrollLists.map(
              (singleAccommodation: AccommodationProps) => (
                <style.AccommodationContentLink
                  to={`/accommodation/${singleAccommodation.id}`}
                  key={singleAccommodation.id}
                  onClick={() =>
                    setRelatedCateRegion({
                      category: singleAccommodation.category,
                      region: singleAccommodation.region,
                    })
                  }
                >
