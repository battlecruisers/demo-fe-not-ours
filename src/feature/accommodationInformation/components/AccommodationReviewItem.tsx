import * as style from '../styles/accommodationReviewList';
  ...
  return (
    <style.SlideItemWrapper>
      <style.ReviewBox>
        ...
        <style.TextInfo>
          <span>{writer}</span>
          <span>{`${date.getFullYear()}-${('0' + (date.getMonth() + 1)).slice(
            -2,
        ...
        <style.ContentWrapper>
          ...
          <style.RoomImage
            src={`https://ybe-bucket.s3.ap-northeast-2.amazonaws.com/ROOM_OPTION_MAIN_2_main_image_1.png`}
          ></style.RoomImage>
        </style.ContentWrapper>
      </style.ReviewBox>
    </style.SlideItemWrapper>
...
const RateStar = (totalRate: number) => {
  const arr = [];
  const filledStarCount = totalRate % 6;

  for (let i = 1; i <= 5; i++) {
    if (filledStarCount >= i) {
      arr.push(
        <svg
          width="14"
          height="14"
